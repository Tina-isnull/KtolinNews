...
public class PullToRefreshRecycler extends RecyclerView implements Pullable {
    ...
    public PullToRefreshRecycler(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public PullToRefreshRecycler(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public boolean canPullDown() {
        ...
        //如果再这里空指针了,回头看看是不是美誉设置layoutmanager--------by chong
        int count = getAdapter().getItemCount();
        if (0 == count) {
            // 没有item的时候也可以下拉刷新
            return true;
        } else if (view!=null&&view.getTop()==20&&mFirstVisiblePosition == 0) {
            // 滑到ListView的顶部了
            return true;
        } else
            return false;
